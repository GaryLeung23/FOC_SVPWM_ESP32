...
PIDController current_loop_M0 = PIDController { .P = 1.2, .I = 0, .D = 0, .ramp = 100000, .limit = 12 };
...
PIDController current_loop_M1 = PIDController { .P = 1.2, .I = 0, .D = 0, .ramp = 100000, .limit = 12 };
// AS5600
...
void M0_setTorque(float Uq, float angle_el)
{
    //使用Uq,Ud合成Uref,并投射到αβ轴上。
    //Uref的模 =sqrt(Uq^2 + Ud^2) 
    //这里angle_el是用于反park变化的，但是公式中的theta是合成矢量到α轴的夹角，所以需要加上atan2(Uq,Ud)
    //这里Ud = 0,atan2(Uq,Ud) = PI/2
    if(Uq<0)
        angle_el+=_PI;
    Uq = abs(Uq);

    float Uout = sqrtf(Uq * Uq) / voltage_power_supply;
    angle_el = _normalizeAngle(angle_el + _PI_2);

    //最大Uref=  2/3 * Udc * sqrt(3) / 2 = sqrt(3)/3 * Udc 
    if (Uout > _1_SQRT3)
        Uout = _1_SQRT3;
    else if (Uout < -_1_SQRT3)
        Uout = -_1_SQRT3;
        
    int sector = floor(angle_el / _PI_3) + 1;
    // calculate the duty cycles
    float T1 = _SQRT3 * sin(sector * _PI_3 - angle_el) * Uout;
    float T2 = _SQRT3 * sin(angle_el - (sector - 1.0) * _PI_3) * Uout;
    float T0 = 1 - T1 - T2;
...
void DFOC_enable()
{
    ...
    digitalWrite(M0_enablePin, HIGH);
}

void DFOC_disable()
{
    digitalWrite(M1_enablePin, LOW);
    digitalWrite(M0_enablePin, LOW);
}
...
void DFOC_Vbus(float power_supply)
{
    ...
    pinMode(M0_enablePin, OUTPUT); // 设置使能引脚
